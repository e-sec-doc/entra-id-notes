...
•	Attribute-Based Access Control (ABAC) adds a layer to the RBAC capabilities. ABAC adds conditions that allow or deny user access to resources. Access is given only if the conditions are met at the time of access. For example a condition could be the time and location of access, or if the user has access to a given project or department; only allow access to Finance app if Department=Finance AND device is compliant. 
